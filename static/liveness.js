...
const TASKS = [
  {
    ...
    check: (history) => detectYawShift(history, 20),
  },
  {
    id: "turn-right",
  ...
  },
  {
    ...
    check: (history) => detectNod(history),
  },
  {
    id: "blink-twice",
    instruction: "Blink twice",
    check: (history) => detectDoubleBlink(history),
  },
  {
    id: "move-closer",
    instruction: "Move closer then back",
    check: (history) => detectDistanceChange(history),
  },
];
...
export function processFrame(session, video, timestampMs) {
  if (session.failed || session.currentIndex >= session.tasks.length) {
    return session;
  }
...
function isSuspicious(history, taskId) {
  if (history.length < 5) return false;

  const deltas = [];
  for (let i = 1; i < history.length; i++) {
    const dYaw = Math.abs(
      history[i].headPose.yaw - history[i - 1].headPose.yaw
    );
    const dPitch = Math.abs(
      history[i].headPose.pitch - history[i - 1].headPose.pitch
    );

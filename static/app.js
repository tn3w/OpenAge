import { startCamera, captureFrame, stopCamera, checkFrameQuality }
  from "./camera.js";
import { ensureModels, getMediaPipeModelBuffer, clearCache }
  from "./model-store.js";
import { initTracker, track, destroyTracker }
  from "./face-tracker.js";
import {
  createLivenessSession, processFrame,
  isLivenessComplete, currentInstruction, currentTaskId,
  progress,
} from "./liveness.js";
import {
  initAgeEstimator, estimateAgeBurst, modelVersion,
} from "./age-estimator.js";
...
function startPositioning() {
  ...
  const check = () => {
    ...
    const result = track(
      elements.video, performance.now()
    );
    ...
    if (!result || result.faceCount === 0) {
      ...
      setVideoStatus("Multiple faces detected — only one person");
      stableFrames = 0;
    } else {
      ...
      if (!quality.ok) {
        ...
        drawFaceOutline(result.boundingBox);
    ...
    setTimeout(check, POSITION_CHECK_INTERVAL_MS);
  };
...
function updateChallengeUI() {
  ...
  setVideoStatus(
    `Challenge ${livenessSession.currentIndex + 1}` +
    ` of ${livenessSession.tasks.length}`
  );
}
...
function handleLivenessFail() {
  ...
  if (retryCount >= MAX_RETRIES) {
    showResult("fail", "Unable to verify. Please try again later.");
    reportResult({ outcome: "unable_to_verify" });
    return;
  }
  showResult(
    "retry",
    `Verification unsuccessful. ${MAX_RETRIES - retryCount}` +
    ` attempt(s) remaining.`
  );
}
...
function handleRetryDecision() {
  ...
  showResult(
    "retry",
    `Could not confirm age. ${MAX_RETRIES - retryCount}` +
    ` attempt(s) remaining.`
  );
}
...
async function reportResult(decision) {
  const payload = {
    modelVersion: modelVersion(),
    decision: decision.outcome,
    retryCount,
    livenessTasksCompleted: livenessSession?.completedTasks ?? 0,
...
async function onClear() {
  ...
  elements.status.textContent =
    "Data cleared. Reload to start again.";
  elements.startButton.classList.add("hidden");
  elements.retryButton.classList.add("hidden");
}
...
function clearOverlay() {
  ...
  context.clearRect(
    0, 0, elements.overlay.width, elements.overlay.height
  );
}

function drawFaceOutline(boundingBox) {
  clearOverlay();
}
